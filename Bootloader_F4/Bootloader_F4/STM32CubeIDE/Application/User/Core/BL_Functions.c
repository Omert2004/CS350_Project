...
uint8_t  rx_buf[RX_CHUNK_BYTES]__attribute__((aligned(4)));
uint32_t buf_idx = 0;
uint8_t  byte;
...
uint8_t BL_WriteFromPython(void) {
	memset(&rx_buf,0x0,RX_CHUNK_BYTES);
    ...
    HAL_UART_Receive(&huart1,
                     ...
                     HAL_MAX_DELAY);
	tfp_printf("fw_size dec=%u hex=%x\r\n", firmware_size, firmware_size);
	tfp_printf("SIZE OK\n");

	// --- ADIM 3: Veri Transferi ---
	current_chunk_idx = 0;
	total_received = 0;
	flash_addr = startaddr; // Adresi sifirla
	HAL_Delay(200); //waitpyhton to synchronize
	tfp_printf(" %x\r\n",startaddr);

	while (total_received < firmware_size)
	{
		// 1. Python'dan veri iste
		tfp_printf("A\n");

		// 2. Veriyi al (1 byte)
		if (HAL_UART_Receive(&huart1, &rx_buf[current_chunk_idx], 1, 1000) == HAL_OK)
		{
			current_chunk_idx++;
			total_received++;

			// 3. Buffer Doldu mu VEYA Dosya Bitti mi?
			if (current_chunk_idx == RX_CHUNK_BYTES)
			{
				...
				{
				    uint32_t words = (current_chunk_idx + 3) / 4;
...
term_cmd_t BL_ReadTerminalCommand(uint32_t timeout_ms)
{
    ...
    printf("  r: rollback\r\n");
    printf("  i: info\r\n");
    printf("[BL] Enter choice within %d : ", (int)timeout_ms);
